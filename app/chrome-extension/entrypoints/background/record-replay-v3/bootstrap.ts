 ...
 *
 * Design description:
 * - You must first execute recoverFromCrash() and then start scheduler.start()
 * - Use global singleton keepalive-manager to avoid multiple controller conflicts
...
/**
 * Resolve the Tab ID required to run Run
...
/**
 * Create a default RunExecutor
 * Use RunRunner to execute Flow
 */
function createDefaultRunExecutor(deps: {
  ...
  return async (item: RunQueueItem): Promise<void> => {
    ...
    // 4. Synchronize attempt to RunRecord
    try {
      await deps.storage.runs.patch(runId, {
    ...
    try {
      ...
      // Registered to RunnerRegistry for use by DebugController and RPC
