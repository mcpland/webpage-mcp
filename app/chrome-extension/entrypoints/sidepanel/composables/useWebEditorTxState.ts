 ...
 *
 * Architecture:
 * - The composable should be initialized once at the top-level chat container
 * - Reuse the same instance in children to avoid duplicate listeners
 */
import { computed, onMounted, onUnmounted, ref, type InjectionKey } from '@/entrypoints/shared/reactivity';
...
export interface UseWebEditorTxStateOptions {
   ...
   * Useful when running outside component lifecycle hooks.
   */
 ...
 * Use this shared key to avoid duplicate listener registration.
 */
