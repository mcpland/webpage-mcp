...
function loadRecorder(): {
  ...
  const sendMessage = vi.fn(
    (message: any, callback?: (response: unknown) => void) => {
  ...
  const listener = vi
    ...
    .mock.calls.at(-1)?.[0];
  if (!listener)
    throw new Error("Recorder runtime listener was not registered");
...
describe("recorder iframe ingest boundary", () => {
  ...
  it.each([
    ...
    "does not forward a %s iframe forged script step",
    (_label, origin, src) => {
  ...
  it("rebinds paused and recording state without resetting the recorder sequence", async () => {
    const { listener, sendMessage } = loadRecorder();
    activeListener = listener;
    const frame = document.createElement("iframe");
    frame.id = "recover-frame";
    document.body.appendChild(frame);

    const sendFrameContext = (suffix: string) => {
      const frameEventId = `frame_${suffix.repeat(32)}`;
      listener(
        {
          action: "rr_register_iframe_event",
          sessionId: "sess-frame-test",
    ...
    sendFrameContext("a");
    const firstEvent = sendMessage.mock.calls.find(
      ([message]) => message?.type === "rr_recorder_event",
    )?.[0];
    expect(firstEvent?.meta?.seq).toBe(1);
    sendMessage.mockClear();

    const recover = (desiredStatus: "paused" | "recording") =>
      new Promise<any>((resolve) => {
        listener(
          {
            action: "rr_recorder_control",
            cmd: "recover",
            meta: { sessionId: "sess-frame-test", desiredStatus },
          },
          {} as chrome.runtime.MessageSender,
          resolve,
        );
      });
    await expect(recover("paused")).resolves.toEqual({
      success: true,
      active: true,
      state: "paused",
    });
    await expect(recover("recording")).resolves.toEqual({
      success: true,
      active: true,
      state: "recording",
    });
    ...
    sendFrameContext("b");
    const secondEvent = sendMessage.mock.calls.find(
      ([message]) => message?.type === "rr_recorder_event",
    )?.[0];
    expect(secondEvent?.meta?.seq).toBe(2);
  });
  ...
  it("does not mint predictable frame capabilities when Web Crypto fails", () => {
    const source = readFileSync(
      join(process.cwd(), "inject-scripts", "recorder.js"),
      "utf8",
    );

    expect(source).not.toContain("Math.random().toString(16)");
    expect(source).toMatch(
      /_newFrameEventId\(\)[\s\S]*?catch \{[\s\S]*?return (?:''|"");/,
    );
  });
