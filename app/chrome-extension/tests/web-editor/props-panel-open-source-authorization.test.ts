...
async function mountedPanel() {
  ...
  const openButton = container.querySelector<HTMLButtonElement>('.we-props-source-btn');
...
describe('Props panel open-source authorization', () => {
  beforeEach(() => {
    ...
    authorizationMocks.authorizePrivilegedUiAction.mockResolvedValue('open-source-token');
  ...
  it('does not request a capability for a synthetic page event', async () => {
    ...
    expect(authorizationMocks.authorizePrivilegedUiAction).not.toHaveBeenCalled();
    expect(chrome.runtime.sendMessage).not.toHaveBeenCalledWith(
      expect.objectContaining({ type: BACKGROUND_MESSAGE_TYPES.WEB_EDITOR_OPEN_SOURCE }),
    );
    panel.dispose();
  });
