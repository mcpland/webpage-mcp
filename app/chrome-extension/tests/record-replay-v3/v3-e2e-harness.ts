...
export interface V3E2EHarnessOptions {
  ...
  /** Whether to automatically start the scheduler (default true) */
...
export interface V3E2EHarness {
  ...
  /** Wait for Run to reach final state */
  ...
  /** List all events of Run */
  listEvents(runId: RunId): Promise<RunEvent[]>;

  /** Destroy the harness and release resources */
 ...
 * @description A simple test node that supports success/failure/delay
 */
function createTestNodeDefinition(): NodeDefinition<'test', JsonObject> {
  return {
    ...
    schema: z
      ...
      .passthrough() as unknown as NodeDefinition<'test', JsonObject>['schema'],
...
export function createV3E2EHarness(options: V3E2EHarnessOptions = {}): V3E2EHarness {
  ...
  async function waitForEvent(
    ...
    // Fast-path: Check for persisted events
    try {
...
/**
 * Create RunExecutor for E2E testing
 */
