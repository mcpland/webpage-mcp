...
describe('Quick Panel command surface queue', () => {
  ...
  beforeEach(async () => {
    ...
    const { initQuickPanelCommands } = await import(
      '@/entrypoints/background/quick-panel/commands'
    );
  ...
  it('serializes same-tab toggles and cleans up only the session each attempt created', async () => {
    ...
    expect(authorizationMocks.stopPrivilegedUiSurfaceSession).not.toHaveBeenCalledWith(
      PRIVILEGED_UI_SURFACES.QUICK_PANEL,
      7,
      firstSessionId,
    );
