...
describe('Web Editor session payload boundaries', () => {
  ...
  async function send(type: string, payload: unknown): Promise<Record<string, unknown>> {
    const sendResponse = vi.fn();
    expect(requestListener!({ type, payload }, contentSender(), sendResponse)).toBe(true);
  ...
  it('stores and broadcasts only the TX_CHANGED schema fields', async () => {
    ...
    await expect(send(BACKGROUND_MESSAGE_TYPES.WEB_EDITOR_TX_CHANGED, payload)).resolves.toEqual({
      success: true,
    });
  ...
  it('rejects invalid TX fields and total bytes before session storage', async () => {
    const invalidAction = await send(BACKGROUND_MESSAGE_TYPES.WEB_EDITOR_TX_CHANGED, {
      ...txPayload(),
      action: 'execute-arbitrary-object',
    });
    expect(invalidAction).toMatchObject({ success: false, error: expect.stringMatching(/action/) });

    const oversized = await send(BACKGROUND_MESSAGE_TYPES.WEB_EDITOR_TX_CHANGED, {
      ...txPayload(),
  ...
  it('rejects an oversized selection locator before storage or rebroadcast', async () => {
    const payload = selectionPayload();
    (payload.selected as Record<string, any>).locator.selectors = Array(17).fill('.selector');

    const response = await send(BACKGROUND_MESSAGE_TYPES.WEB_EDITOR_SELECTION_CHANGED, payload);
