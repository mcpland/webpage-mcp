...
describe("Web Editor route resource boundaries", () => {
  ...
  it("uses a bounded client request ID instead of trusting an oversized server field", async () => {
    ...
    expect(new TextEncoder().encode(requestId).byteLength).toBeLessThanOrEqual(
      384,
    );
    const actRequest = nativeHostMocks.requestAgentRpcFetch.mock.calls.find(
      ([request]) => request.operation === "agent.chat.act",
    )?.[0];
    expect(JSON.parse(actRequest.body).requestId).toBe(requestId);
    expect(requestId).not.toBe("x".repeat(1025));
  });
});
