...
import { getFlow, saveFlow } from '@/entrypoints/background/record-replay/flow-store';
import { flowAnalyzeTool, flowUpdateTool } from '@/entrypoints/background/tools/flow-tools';
...
function parseToolPayload(result: { content?: Array<{ type?: string; text?: string }> }): any {
...
describe('recording/editing/flow toolchain integration', () => {
  ...
  it('recordingStartTool forwards trimmed metadata and returns a state snapshot', async () => {
    ...
    const result = await recordingStartTool.execute({ name: '  Signup Flow  ', tabId: 17 });

    expect(mocks.recorderStart).toHaveBeenCalledWith({ name: 'Signup Flow' }, 17);
    ...
    const recordedFlow = createFlow(flowId, [
      {
        ...
        type: 'fill',
        config: {
          target: { selector: '#email', candidates: [{ type: 'css', value: '#email' }] },
  ...
  it('flowAnalyzeTool summarizes saved flows and surfaces recording quality hints', async () => {
    ...
    const payload = parseToolPayload(result);
    const codes = new Set(payload.hints.map((hint: { code: string }) => hint.code));
  ...
  it('flowUpdateTool applies parameter suggestions and persists the edited flow', async () => {
    ...
    expect((updated?.nodes?.find((node) => node.id === 'nav-1')?.config as { url?: string })?.url).toBe(
      'https://example.com/search?q={q}',
    );
    expect(
      (updated?.nodes?.find((node) => node.id === 'fill-1')?.config as { value?: string })?.value,
    ).toBe('{email}');
    expect(updated?.variables?.map((variable) => variable.key)).toEqual(['q', 'email']);
  });
