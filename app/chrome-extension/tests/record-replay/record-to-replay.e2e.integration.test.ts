...
import { recordingStartTool, recordingStopTool } from '@/entrypoints/background/tools/recording';
...
describe('record to replay automation', () => {
  ...
  beforeEach(() => {
    ...
    mocks.handleCallTool.mockImplementation(async ({ name, args }: { name: string; args?: any }) => {
      switch (name) {
        case TOOL_NAMES.BROWSER.NAVIGATE:
          ...
          return {
            content: [{ type: 'text', text: JSON.stringify({ success: true, tabId: TAB_ID }) }],
            isError: false,
          };
    ...
    vi.mocked(chrome.storage.local.get).mockImplementation(async (keys?: string | string[] | object | null) => {
      ...
      if (keys && typeof keys === 'object') {
        ...
        for (const [key, fallback] of Object.entries(keys)) {
          out[key] = key in localStorageState ? localStorageState[key] : fallback;
        }
        return out;
      }
  ...
  it('records a flow and replays the saved recording automatically', async () => {
    ...
    expect(savedFlow?.nodes?.map((node) => node.type)).toEqual(['navigate', 'fill']);
