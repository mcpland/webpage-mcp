 ...
 *
 * Environment:
 * - WEBPAGE_MCP_AGENT_DATA_DIR: overrides the default base directory.
 */
export function getAgentDataDir(): string {
  const raw = process.env.WEBPAGE_MCP_AGENT_DATA_DIR;
  if (raw && raw.trim()) {
 ...
 *
 * Environment:
 * - WEBPAGE_MCP_AGENT_DB_FILE: overrides the default database path.
 */
export function getDatabasePath(): string {
  const raw = process.env.WEBPAGE_MCP_AGENT_DB_FILE;
  if (raw && raw.trim()) {
    return path.resolve(raw.trim());
  }
 ...
 * - Mac/Linux: ~/.webpage-mcp-agent/workspaces
 * - Windows: %USERPROFILE%\.webpage-mcp-agent\workspaces
 */
