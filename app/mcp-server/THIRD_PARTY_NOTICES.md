...
This table is a human-readable summary of the `webpage-mcp` package's direct production dependencies. The exact, machine-readable pnpm production closure—including transitive, peer, optional, and cross-platform packages—is distributed as `THIRD_PARTY_COMPONENTS.json`. npm installs those components as separate packages; the server bundles only first-party `webpage-mcp-shared` code into selected entry points. Listing a build or install input does not imply that its code is bundled, and does not alter its upstream terms.
