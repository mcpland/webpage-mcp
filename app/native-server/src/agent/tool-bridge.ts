...
export interface CliToolInvocation {
   ...
   * When omitted, this bridge defaults to the local webpage MCP server.
   */
  server?: string;
  /**
...
export interface AgentToolBridgeOptions {
   ...
   * If omitted, DEFAULT_SERVER_PORT from webpage-mcp-shared is used.
   */
  mcpUrl?: string;
}
 ...
 * against the local webpage MCP server via the official MCP SDK client.
 *
 * Note:The bridging layer is responsible for uniformly converting tool calls reported by the CLI into standard MCP CallTool requests.
 * Reuse the existing /mcp HTTP server instead of developing additional protocols within this project.
 */
