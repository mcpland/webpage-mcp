...
function resolveNodeCandidate(distDir: string): NodeResolutionResult {
  ...
  // Priority 0: WEBPAGE_MCP_NODE_PATH
  const fromEnv = consider('WEBPAGE_MCP_NODE_PATH', process.env.WEBPAGE_MCP_NODE_PATH);
  if (fromEnv) {
...
export async function collectDoctorReport(options: DoctorOptions): Promise<DoctorReport> {
  ...
  const packageName = typeof pkg.name === 'string' ? pkg.name : 'webpage-mcp-bridge';
  ...
  const nodePathWarn =
    ...
    !process.env.WEBPAGE_MCP_NODE_PATH;
  ...
  if (!nodeResolution.nodePath) {
    ...
    nodeFix = [
      ...
      `Or set WEBPAGE_MCP_NODE_PATH to an absolute node path`,
    ];
