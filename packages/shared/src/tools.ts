...
/**
 * Public and runtime ceilings for chrome_javascript. Keeping these values in
 * the shared package prevents clients from advertising inputs that the
 * extension cannot safely execute or return.
 */
export const JAVASCRIPT_TOOL_LIMITS = Object.freeze({
...
export const TOOL_SCHEMAS: Tool[] = [
    ...
    inputSchema: {
      ...
      properties: {
        ...
        action: {
          type: "string",
          description:
            "Action to perform: left_click | right_click | double_click | triple_click | left_click_drag | scroll | scroll_to | type | key | fill | fill_form | hover | wait | resize_page | zoom | screenshot. All page-targeting actions are only supported on HTTP(S) pages; `resize_page` and pure timer waits are exempt.",
        },
        ref: {
          type: "string",
          description:
  ...
  // },
  {
    ...
    description:
      "Execute JavaScript code in an HTTP(S) browser tab and return the result through the Chrome Debugger API. If another debugger owns the tab, the call fails without executing the code. Results are sanitized and serialized under strict page-side depth, width, string, node, and byte budgets before crossing the debugger boundary.",
    inputSchema: {
      type: "object",
      additionalProperties: false,
      properties: {
        code: {
          type: "string",
          maxLength: JAVASCRIPT_TOOL_LIMITS.MAX_CODE_BYTES,
          description: `JavaScript code to execute (maximum ${JAVASCRIPT_TOOL_LIMITS.MAX_CODE_BYTES} UTF-8 bytes). Runs inside an async function body, so top-level await and "return ..." are supported.`,
        },
        tabId: {
          type: "number",
          description:
        ...
        maxOutputBytes: {
          ...
          description:
            "Maximum UTF-8 output size after sanitization. Output exceeding this limit is truncated in the page before CDP returns it.",
        },
      },
      required: ["code"],
    },
