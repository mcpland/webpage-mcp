...
REM Windows: %LOCALAPPDATA%\webpage-mcp-bridge\logs
set "LOG_DIR=%LOCALAPPDATA%\webpage-mcp-bridge\logs"
...
echo WEBPAGE_MCP_NODE_PATH: %WEBPAGE_MCP_NODE_PATH% >> "%WRAPPER_LOG%"
...
REM Priority 0: WEBPAGE_MCP_NODE_PATH environment variable override
echo [Priority 0] Checking WEBPAGE_MCP_NODE_PATH override >> "%WRAPPER_LOG%"
if defined WEBPAGE_MCP_NODE_PATH (
    ...
    if exist "!CANDIDATE_NODE!" (
        ...
        echo Found node via WEBPAGE_MCP_NODE_PATH: !NODE_EXEC! >> "%WRAPPER_LOG%"
    ) else (
        echo WEBPAGE_MCP_NODE_PATH is set but not found: !CANDIDATE_NODE! >> "%WRAPPER_LOG%"
    )
)
...
if not defined NODE_EXEC (
    ...
    echo Searched: WEBPAGE_MCP_NODE_PATH, node_path.txt, relative, Volta, asdf, fnm, where, common paths >> "%WRAPPER_LOG%"
    echo To fix: Set WEBPAGE_MCP_NODE_PATH environment variable or run 'webpage-mcp-bridge doctor --fix' >> "%WRAPPER_LOG%"
    exit /B 1
)
