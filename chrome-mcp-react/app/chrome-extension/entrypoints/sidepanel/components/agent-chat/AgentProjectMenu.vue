<template>
  ...
  >
    ...
    <!-- Webpage MCP Option - only shown when Claude or Codex CLI is selected -->
    <div v-if="showWebpageMcpOption" class="px-3 py-2 flex items-center gap-2">
...
// Show Webpage MCP option when Claude, Codex, or Auto (empty) CLI is selected
// Auto typically defaults to Claude, and users should be able to manage this project-level setting
const showWebpageMcpOption = computed(() => {
...
function handleWebpageMcpChange(event: Event): void {
  emit('webpage-mcp:update', (event.target as HTMLInputElement).checked);
}
